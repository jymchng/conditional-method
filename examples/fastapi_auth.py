...
class AuthService:
    ...
    def authenticate_user(
        ...
        try:
            payload = jwt.decode(
                authorization.credentials, get_secret_key(), algorithms=[JWT_ALGORITHM]
            )
...
@app.get("/protected")
def protected_route(
    user_data: Dict[str, Any] = Depends(AuthService().authenticate_user),
):
    return {
...
def admin_only(user_data: Dict[str, Any] = Depends(AuthService().authenticate_user)):
    ...
    if (
        os.environ[ENVIRONMENT_KEY] == "development"
